...
class TVVerticalAdapter(private val tvResults: List<TVResult>, private val context: Context) :
    RecyclerView.Adapter<TVVerticalAdapter.TvViewHolder>() {
    ...
    class TvViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItem(tvResult: TVResult, context: Context) {
            if (!TextUtils.isEmpty(tvResult.posterPath)) {
                ...
                ImageAdapter.setPosterURL(imageItemPoster, tvResult.posterPath)
            } else {
                loadingVerticalItemImage.visibility = View.GONE
